...
extension ExampleSchema {
	open class StargazerConnectionQuery: GraphQL.AbstractQuery, GraphQLQuery {
		public typealias Response = StargazerConnection

		@discardableResult
		open func edges(aliasSuffix: String? = nil, _ subfields: (StargazerEdgeQuery) -> Void) -> StargazerConnectionQuery {
			let subquery = StargazerEdgeQuery()
			subfields(subquery)
		...
		@discardableResult
		open func nodes(aliasSuffix: String? = nil, _ subfields: (UserQuery) -> Void) -> StargazerConnectionQuery {
		...
		@discardableResult
		open func pageInfo(aliasSuffix: String? = nil, _ subfields: (PageInfoQuery) -> Void) -> StargazerConnectionQuery {
		...
		@discardableResult
		open func totalCount(aliasSuffix: String? = nil) -> StargazerConnectionQuery {
			addField(field: "totalCount", aliasSuffix: aliasSuffix)
			return self
		}
	}

	open class StargazerConnection: GraphQL.AbstractResponse, GraphQLObject {
		public typealias Query = StargazerConnectionQuery
		...
		open override func deserializeValue(fieldName: String, value: Any) throws -> Any? {
			...
			switch fieldName {
				...
				guard let value = value as? [Any] else {
					throw SchemaViolationError(type: StargazerConnection.self, field: fieldName, value: fieldValue)
				}
				return try value.map { if $0 is NSNull { return nil }
				guard let value = $0 as? [String: Any] else {
					throw SchemaViolationError(type: StargazerConnection.self, field: fieldName, value: fieldValue)
				}
				return try StargazerEdge(fields: value) } as [Any?]
		...
		open var typeName: String { return "StargazerConnection" }

		open var edges: [ExampleSchema.StargazerEdge?]? {
			return internalGetEdges()
		}

		func internalGetEdges(aliasSuffix: String? = nil) -> [ExampleSchema.StargazerEdge?]? {
			return field(field: "edges", aliasSuffix: aliasSuffix) as! [ExampleSchema.StargazerEdge?]?
		}
