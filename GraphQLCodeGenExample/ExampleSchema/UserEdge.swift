...
extension ExampleSchema {
	open class UserEdgeQuery: GraphQL.AbstractQuery, GraphQLQuery {
		public typealias Response = UserEdge

		@discardableResult
		open func cursor(aliasSuffix: String? = nil) -> UserEdgeQuery {
			addField(field: "cursor", aliasSuffix: aliasSuffix)
			return self
		}

		@discardableResult
		open func node(aliasSuffix: String? = nil, _ subfields: (UserQuery) -> Void) -> UserEdgeQuery {
	...
	open class UserEdge: GraphQL.AbstractResponse, GraphQLObject {
		public typealias Query = UserEdgeQuery
		...
		open override func deserializeValue(fieldName: String, value: Any) throws -> Any? {
			...
			switch fieldName {
				...
				guard let value = value as? String else {
					throw SchemaViolationError(type: UserEdge.self, field: fieldName, value: fieldValue)
				}
				return value
		...
		open var typeName: String { return "UserEdge" }
